    ...
    return (
        <>
            ...
            <div className={category_content}>
                ...
                {posts.edges.length === 0 ? <p>No posts to see here...</p> : posts.edges.map(({ node }) => <CategoryPost className={content_container} post={node} categorySlug={slug} key={node.slug} />)}
                {currentPage > 1 ? fetchedData.map(({ node }) => <CategoryPost className={content_container} post={node} categorySlug={slug} key={node.slug} />) : null}
